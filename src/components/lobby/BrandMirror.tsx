  ...
  return (
    <div className="h-full overflow-y-auto bg-[var(--bg-base)]">
      <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-6">
        ...
        {/* Style Preview — what the AI sees */}
        {stylePreview && stylePreview.prompt && (
          <div className="rounded-2xl p-5" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
            <h3 className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-3">What Your AI Sees</h3>
            <p className="text-[11px] text-[var(--text-muted)] mb-3">
              This is the creative direction your brand settings generate for the styling engine.
            </p>
            <div className="rounded-xl p-4" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] font-medium px-2 py-0.5 rounded-full"
                  style={{ background: 'var(--accent)', color: 'var(--bg-base)', opacity: 0.9 }}>
                  {stylePreview.style_name}
                </span>
                <span className="text-[10px] text-[var(--text-muted)]">
                  Strength: {Math.round(stylePreview.strength * 100)}%
                </span>
              </div>
